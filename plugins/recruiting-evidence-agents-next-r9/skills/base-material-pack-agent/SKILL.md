...
2. 选定表后，运行 `python3 scripts/build_base_materials.py --base-url "<BASE_URL>" --table-id "<SELECTED_TABLE_ID>" --list-batches`。对业务人员展示批次名称与记录数，并在批次选择前明确说明：默认会保存到桌面；每个批次将在所选工作目录直接生成一个固定命名的文件夹，例如 `招聘材料包_第1批_YYYYMMDD_HHmm`。使用者可回复“桌面”确认默认位置，或提供一个已存在的本地工作目录；系统只会在该目录下生成上述固定命名文件夹，不能自定义或更改材料包文件夹名。使用以下固定话术：`该表包含：<批次>（<记录数> 人）。请选择需要处理的范围，并确认输出位置：默认保存到桌面；也可提供一个本地工作目录。材料包会命名为 招聘材料包_第N批_YYYYMMDD_HHmm。` 多批次时逐项列出；单批次也必须等待批次和输出位置确认。不要显示 ID 或命令。
...
5. 设 `WORKSPACE_ROOT` 为已选工作目录，`OUTPUT_DIR` 为 `WORKSPACE_ROOT/招聘材料包_<业务批次标签>_<采集时间>`，并将快照与催办控制目录置于 `WORKSPACE_ROOT/.招聘材料控制/`。先由原始批次得出业务批次标签，例如 `Batch 1 -> 第1批`，再构造该目录；不得把原始 Base 批次直接拼入输出目录。该 `WORKSPACE_ROOT` 与 `OUTPUT_DIR` 是后续补件刷新唯一允许的位置，不能在监听时改用会话工作目录。Run:
