...
def reply_matches_sent_item(item: dict[str, Any], message: dict[str, Any]) -> bool:
    if message.get("message_id") == item.get("sent_message_id") or message.get("deleted"):
        return False
    ...
    return (
        ...
        or message.get("reply_to") == sent_message_id
        or message.get("parent_id") == sent_message_id
        or message.get("root_id") == sent_message_id
    )
...
def is_unthreaded_acknowledgement(item: dict[str, Any], message: dict[str, Any]) -> bool:
    ...
    if (
        message.get("message_id") == item.get("sent_message_id")
...
def confirmed_missing_matches(item: dict[str, Any], message: dict[str, Any]) -> bool:
    ...
    return (
        ...
        and (message.get("reply_to") == sent_message_id or message.get("root_id") == sent_message_id)
    )
...
def acknowledge_replies(state: dict[str, Any], messages_by_recipient: dict[str, list[dict[str, Any]]]) -> list[dict[str, Any]]:
    ...
    used_message_ids.update(
        ...
        for item in state["items"]
        if isinstance(item.get("sent_message_id"), str)
    )
...
def refresh(control_dir: Path, base_url: str, snapshot_root: Path, handoff_root: Path, profile: str, download_workers: int) -> dict[str, Any]:
    ...
    handoff_dir = (handoff_root / f"招聘材料包_{run_control.business_batch_label(batch)}_{stamp}").resolve()
    if snapshot_dir.exists() or handoff_dir.exists():
    ...
    command = [
        ...
        "--output-dir", str(handoff_dir), "--table-id", table_id, "--batch", batch, "--lark-profile", profile,
...
def watch(
    ...
    try:
        ...
        reminders = reminder(control_dir, profile) if timed_out else []
        completion = manage_follow_up.record_watch_completion(
            control_dir, timed_out=timed_out, refreshes=len(refreshes), reminders=reminders,
        )
        result = {
            "action": "watch",
            ...
            "refreshes": refreshes,
            "unanswered_items": unanswered_items(control_dir),
