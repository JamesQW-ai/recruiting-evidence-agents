...
def load_state(control_dir: Path) -> dict[str, Any]:
    ...
    if state.get("schema_version") not in {1, 2, CONTROL_SCHEMA} or not isinstance(state.get("items"), list):
...
def record_watch_completion(control_dir: Path, *, timed_out: bool, refreshes: int, reminders: list[dict[str, str]]) -> dict[str, Any]:
    with control_lock(control_dir):
        state = load_state(control_dir)
...
def dispatch_and_watch(
    ...
    if not planned:
        ...
        return {
            ...
            "task_heartbeat_required": False, **synced,
        }
...
def main() -> int:
    ...
    dispatch_parser = subparsers.add_parser("dispatch-and-watch")
    ...
    if args.action == "plan":
        ...
        result = prepare_preview(snapshot_dir, control_dir, args.base_url, args.table_id, args.lark_profile)
    else:
