...
为每次批次选择一个独立的 `CONTROL_DIR`，且它必须不同于 `SNAPSHOT_DIR` 与正式交付目录。三者均属于已选工作目录：材料包直接位于工作目录根下，快照和控制状态位于工作目录的隐藏控制目录。首次生成预览时，必须把 `WORKSPACE_ROOT` 与当前 `OUTPUT_DIR` 绑定到控制状态；后续监听、恢复监听和刷新不得改用当前工作目录、插件目录或其他输出位置。批次、`candidate_id` 和 Base `record_id` 必须在控制状态中同时保存；同名候选人不得仅按姓名合并。一个 session 追加新批次或同一批次新增候选人时，必须新建对应的 `SNAPSHOT_DIR`、`CONTROL_DIR` 和空输出目录，不能沿用上一个批次的上下文。此目录可保存：
...
```bash
python3 scripts/manage_follow_up.py dispatch-and-wait \
  --control-dir "<CONTROL_DIR>" \
  --base-url "<BASE_URL>" \
  --table-id "<TABLE_ID>" \
  --snapshot-root "<WORKSPACE_ROOT>/.招聘材料控制" \
  --handoff-root "<WORKSPACE_ROOT>" \
  --lark-profile "<LOCAL_PROFILE>" \
  --confirm SEND_AND_WATCH \
  --timeout-seconds 600 \
  --interval-seconds 15 \
  --wait-timeout-seconds 1260
```
...
`dispatch-and-wait` 只启动一个本地有界监听进程，并在同一前台命令中保持等待至终态；不得改用 `dispatch-and-watch`、`wait-for-completion`、自动化、周期性检查或任何中间对话输出，**不得创建 Codex heartbeat**。收到所有合格回复时，监听会提前结束；初始十分钟届满时，未回复项发送一次自动再次催办并进入第二个十分钟监听窗口，第二窗口内的回复可关联原催办或再次催办消息；不会发送第二轮提醒。前台等待上限为 21 分钟，以覆盖两个有限窗口。**在该命令返回前，禁止用 `sleep`、60 秒检查、`read-completion-result` 或任何日志读取替代等待；`watch_status=Running` 不是终态，不能据此结束对话。**无论哪种终态，命令都会原子领取 `follow_up_completion.json` 中的唯一通知。终态会明确说明已发送的再次催办数量；`next_action=await_reminded_replies` 时只返回 `message` 与再次催办状态；`next_action=collect_hrd_opinions` 时命令会立即运行同一受控输出目录的 HRD queue，并在同一 JSON 返回 `hrd_queue`。当前对话仅依据 `terminal` 这一次返回回报结果和 HRD 队列；不得再次读取、领取、解释日志或重复输出。

若前台等待命令因宿主执行环境中断、无输出或无法解析而未提供 JSON，**不得**报告“未收到终态”或重新发送消息。仅允许立即执行一次以下恢复读取；它不等待、不轮询，也不会再发送消息，可从已领取或未领取的完成记录恢复同一终态：

```bash
python3 scripts/manage_follow_up.py read-completion-result \
  --control-dir "<CONTROL_DIR>" \
  --base-url "<BASE_URL>" \
  --table-id "<TABLE_ID>" \
  --lark-profile "<LOCAL_PROFILE>"
...
`read-completion-result` 只能在前台等待命令被宿主中断、无输出或无法解析后执行一次；不得在 60 秒或任意中间时点主动调用。`terminal_ready=true` 时只按该返回汇报一次。`terminal_ready=false` 时只如实说明监听仍在运行或未启动，不能把它写作超时、未收到终态或发送失败；不得创建 heartbeat 或第二次等待。

监听完成后会将结果写入 `CONTROL_DIR/follow_up_completion.json`：全部已发送事项收到合格回复时，`next_action` 为 `collect_hrd_opinions`；十分钟超时仍有未回复事项时，先处理已回复项并对每个未回复事项自动发送一次可审计的再次催办，`next_action` 为 `await_reminded_replies`。每项超时提醒仅有一次新的幂等消息 ID，不无限循环发送。
...
收到一次“确认发送并监听10分钟”后，受控监听进程会立即开始每 15 秒一次的监听：补交事项识别“已上传”“已补交”或在原消息下包含“无法提供”的说明，核查与清理事项识别“已处理”或“已删除”。业务人员应在对应催办消息下回复，以确保回复可归属；同时兼容飞书的 `reply_to`、`parent_id` 与 `root_id` 关联字段。若全部已发送事项都收到合格回复，立即停止监听、统一创建一次新的 Base 只读快照，沿用原已选表和批次，并以临时目录构建后替换首次绑定的 `CURRENT_OUTPUT_DIR`；旧交接归档到隐藏控制目录，因此工作目录始终只有一个可见业务材料包。若少于全部事项回复，则持续至 10 分钟结束；窗口结束后，只要已收到任一合格回复，也统一创建一次新的 Base 只读快照。`Confirmed Missing` 只记录关联原催办消息的“无法提供”说明。未关联原催办消息的单独回复不会触发刷新，避免同一负责人收到多条催办时误关联。已有发送回执但监听因外部中断未启动时，使用 `resume-watch` 仅恢复这批回执，绝不再次发送消息。结束时由唯一的终态通知向使用者汇报，不能重复回显。读取私聊需要用户身份具有 `im:message:readonly` 与 `im:chat:read`，发送还需要 `im:message` 与 `im:message.send_as_user`。缺少权限时停止报告，不能切换为 bot 身份绕过。
